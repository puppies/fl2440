...
{
	struct s3c2440_nand *nand = s3c2440_get_base_nand();
	*(unsigned int *)ecc_code = readl(&nand->NFMECCD0);
	*(unsigned int *)(ecc_code+4) = readl(&nand->NFMECCD1);
	*(unsigned int *)(ecc_code+8) = readl(&nand->NFSECCD);
	debugX(1, "s3c2440_nand_calculate_hwecc(%p,): 0x%x 0x%x 0x%x\n",
	       mtd , *(unsigned int *)ecc_code, *(unsigned int *)(ecc_code+4), *(unsigned int *)(ecc_code+8));
